...
import { X, ChevronLeft, ChevronRight, BookOpen, Heart, Lock } from "lucide-react";
import gargee1 from "@/assets/gallery/gargee-1.jpg";

const DREAMSCAPE_BOOK_AUTH_KEY = "dreamscape-book-unlocked";
const CORRECT_PASSWORD = "iwannabeyoursforever";
...
The only person who truly anchored me during this time was my sister, Sanskriti. She was the one soul I could share everything with the one person who actually listened. Our relationship was built on a foundation of genuine trust, though it was often disguised by the typical sibling dynamic of relentless poking and teasing. She would mock my dramatic stories and roll my eyes at my heartbreaks, but she was always there when the silence became too loud.  
...
export default function BookReader({ open, onClose }: BookReaderProps) {
  ...
  useEffect(() => {
    if (localStorage.getItem(DREAMSCAPE_BOOK_AUTH_KEY) === "true") {
      setUnlocked(true);
    }
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (password === CORRECT_PASSWORD) {
      setUnlocked(true);
      localStorage.setItem(DREAMSCAPE_BOOK_AUTH_KEY, "true");
    } else {
      setError(true);
